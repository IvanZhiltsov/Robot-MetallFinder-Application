...
1. - Имеет виджет *QWebEngineView* для отображеиня карты реальной местности
   ...
   - Также с помощью JavaScript создаются дополнительные графические объекты для отображеия данных на карте
...
5. Режим *курсор*:
   ...
   - Щёленув правой кнопкой мыши по ребру полигона, можно добавить новую вершину на ребре в месте клика
   - Щёлкнув правой кнопкой мыши по вершине полигона, можно удалить её (удаляются 2 выходящих из неё ребра, а 2 соседнии вершины соединяются новым ребром)
...
7. Условие создания полигона:
   ...
   - длина рёбер и угол между ними находятся в рамках некоторых критических значений, кторые учитывают размеры робота и погрешность при его перемещении
...
10. - При загрузке данных с робота, они отображаются на карте
...
11. - Карту можно открыть с памяти компьютера и, если тип карты позволяет, редактировать или загрузить в робота
...
2. Если изначально компьютер не подключён к роботу, то отображается *список доступных устройств*, а *панель связи* и *окно состояния* скрыты
   - Отображаются имена устройств. При клике на них выполняется подклучение к соответствующему роботу
...
3. Если изначально к компьютеру подключён не робот (например наушники), то отключене такого устройства не происходит
...
6. *Панель связи*:
   ...
   - Кнопка *очистить память* удаляет из памяи робота все данные и инструкции и переводит его в *режим ожидания*
7. *Окно загрузки карты:*
   ...
   - Если текущего заряда робота не хватает на выполнение работы, то выводится соответствующее предупреждение в Статус-Баре и дублируется перед окончательной отправке роботу инструкций
...
2. - В памяти компьютера и робота храниться только набор координат (вершины полигона, точка *финиша* и найденные места с металлом), а так же тип карты
