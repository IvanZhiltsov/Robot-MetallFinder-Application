...
Данный проект является частью более сложного проекта по созданию автономного робота-металлоискателя. Задача данного проекта - создать независимое приложение для управления роботом-металлоискателем

В ходе работы было разработано приложение, позволяющее пользователю указывать на карте область поиска металла и отправлять её роботу-металлоискателю, а также просматривать на карте полученные от робота места, в которых был обнаружен металл

Ссылка на Яндекс Диск с готовой программой:
https://disk.yandex.ru/d/iXoZDTNDujEJNA

# Инструкция по установке приложения

**1 способ** - готовое приложение
1. Скачайте и разархивируйте папку с готовым приложением, cсылка на Яндекс Диск:
   - https://disk.yandex.ru/d/iXoZDTNDujEJNA
2. В скаченной папке запустите файл *main.exe*
3. Приложение открылось - можно приступать к работе!

**2 способ** - запустить код через среду программирования
1. Скачайте и разархивируйте данный репозиторий с GitHub
2. Откройте вашу среду программирования, например Pycharm или VisualStuduio
3. Создайте новый проект на осонове скаченного репозитория (добавьте виртуальное окружение)
4. Установите библиотеки из файла requirements.txt, для быстрой установки пропишите в терминале команду:
   - *pip install -r requirements.txt*
5. В папке *APP* запустите файл *main.py*
6. Приложение открылось - можно приступать к работе!
...
1. Основной код для работы программы содержится в отдельной папке *APP*
    - *main.py* - скрипт для запуска приложения
   - *Windows_Templates_UI* - папка с Ui-шаблонами окон
   - *Init_Windows* - папка с классами для инициализации окон
   - *MapPy.py* - класс для реализации карты
   - *MapHTML.html* - html-шаблон для отображания Яндекс Карты
   - *BluetoothPy.py* - класс, реализующий функции bluetooth
   - *ActionsManagerPy.py* - класс, реализующий отмену/возврат действий
2. *Map Tests* - папка с примерами сохранённых карт

**Реализация карты**
- QWebEngineView виджет – запускает браузер с установленным HTML-шаблоном
